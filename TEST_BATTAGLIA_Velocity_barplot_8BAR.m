%% function TEST_BATTAGLIA_Velocity_barplot_8BAR.m
clear; close all;
par.irng = 10;
rng(par.irng)
...
for cd=1:num_cond
    for ndir = 1:num_dir
        ...
        if Vs~=0
            vs_max = max(Vs(MovOnset_ind:end));
            MS_smoothSG_Max(cd).(strcat('dir',num2str(ndir))) = vs_max;
        else
            MS_smoothSG_Max(cd).(strcat('dir',num2str(ndir))) = 0;
        end
        if Vk~=0
            vk_max = max(Vk(MovOnset_ind:end));
            MK_smoothSG_Max(cd).(strcat('dir',num2str(ndir))) = vk_max;
        else
            MK_smoothSG_Max(cd).(strcat('dir',num2str(ndir))) = 0;
        end
    end
end
...
MS_SGplotMax = (cell2mat(struct2cell(MS_smoothSG_Max')))';
MK_SGplotMax = (cell2mat(struct2cell(MK_smoothSG_Max')))';

%% Vmax plot per 8 direzioni per 4 condizioni
Vmax_plotSG = [MS_SGplotMax(1,:);MK_SGplotMax(2,:);MS_SGplotMax(3,:);MK_SGplotMax(3,:)];
max_y = max(max(Vmax_plotSG));
%% BOXPLOT FIGURE
...
Vmax_barplot_path = strcat(dir_path,'\VelocitySmooth\8dir\',num2str(windowSize),'wind\');
