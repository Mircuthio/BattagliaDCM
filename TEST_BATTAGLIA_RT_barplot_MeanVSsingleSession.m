%% TEST_BATTAGLIA_RT_barplot_MeanVSsingleSession

% This is a plot script of mean RT barplot. 
% The plot shows mean RT of a specific session (specified in session_list)
% versus mean RT of all sessions (Frontal and Parietal)

clear; close all;
par.irng = 10;
rng(par.irng)

% session_list contains list of sessions to plot
session_list = {'SK022';'SK025';'SK033';'SK035';'SK036';'SK038';'SK042';'SK043';... %Frontal 8*8*3
    'SK047';..., %Frontal 9*9*3
    'SK051';'SK059';'SK060';'SK062';'SK065';'SK069';'SK074'}; % Parietal 8*8*3

for i=1:length(session_list)

    session_single_name = session_list{i}; % single session to compare
    ...
    %% Selection of all session files
    ...
    % note: selS and selK are releated to LFPs not included in this
    % Behaviour analysis
    RT_session = struct();
    lenTot = struct();
    Bad_session = struct(); %Delete the session with label 0 (code 8003)
    for idsess = 1:length(session_name)
        ...
        %% organize RT by condition: SoloS, SoloK, JointS, JointK
        RT_bar(1) = RT_S(1);
        RT_bar(2) = RT_K(2);
        RT_bar(3) = RT_S(3);
        RT_bar(4) = RT_K(3);
    ...
    max_y = max(max(RT_plotMean+2*RT_plotStd)); % RT max value for axis range

    %% Selection adn Extraction of single session in session_list
    RT_Singlesession = struct();
    lenTot = struct();
    Bad_sessionSingle = struct();
    session_name = session_single_name;
    fprintf('Step 0: arrange trials\n');
    par.BattagliaArrangeTrials              = BattagliaArrangeTrialsParams();
    % par.BattagliaArrangeTrials.whichmodel   = 7;      % 7 Model for action session. 5 Model for all sessions
    par.BattagliaArrangeTrials.isdemo       = 1;        % getSelectionIndexes(1,1:3);
    ...
    % Directory of barplot files
    dir_path = strcat('D:\main_scriptDCM\Figure\',session_name,'\rng',num2str(par.irng),'\');
