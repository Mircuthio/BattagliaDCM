...
for cd=1:num_cond
    for ndir=1:num_dir
        ...
        Jduration = (M_app(k).ET*fs);
        MS_appX(k,:) = M_app(k).(Jfieldname)(1,1:Jduration);
        MS_appY(k,:) = M_app(k).(Jfieldname)(2,1:Jduration);
        MK_appX(k,:) = M_app(k).(Jfieldname)(5,1:Jduration);
        MK_appY(k,:) = M_app(k).(Jfieldname)(6,1:Jduration);
    end
    MS(cd).(strcat('dir',num2str(ndir))) = [MS_appX;MS_appY];
    MK(cd).(strcat('dir',num2str(ndir))) = [MK_appX;MK_appY];
    
end
%% Mean Cursor Movement (x,y)
ET = [data_trials.ET]';
Jduration = 1250;
Jfieldname = 'JXYEXY';
MS = struct();
...
for cd=1:num_cond
    for ndir=1:num_dir
        M_app = Move(cd).(strcat('dir',num2str(ndir)));
        MS_appX = NaN(lenM(cd,ndir),Jduration);
        MS_appY = NaN(lenM(cd,ndir),Jduration);
        MK_appX = NaN(lenM(cd,ndir),Jduration);
        MK_appY = NaN(lenM(cd,ndir),Jduration);        
        ...
        MS(cd).(strcat('dir',num2str(ndir))) = [mean(MS_appX,1);mean(MS_appY,1)];
        MK(cd).(strcat('dir',num2str(ndir))) = [mean(MK_appX,1);mean(MK_appY,1)];
    end
end
...
for cd = 1:num_cond
    tempRT_S = RT_S(cd);
    maxlen = max(lenM(cd));
    fieldsRT_S = fieldnames(tempRT_S);
    RT_Sbox = NaN(maxlen,num_dir);
    for iRTs=1:num_dir
        RTs_app = tempRT_S.(fieldsRT_S{iRTs});
        RT_Sbox(1:length(RTs_app), iRTs) = RTs_app;
    end
    tempRT_K = RT_K(cd);
    maxlen = max(lenM(cd));
    fieldsRT_K = fieldnames(tempRT_K);
    RT_Kbox = NaN(maxlen,num_dir);
    for iRTk=1:num_dir
        RTk_app = tempRT_K.(fieldsRT_K{iRTk});
        RT_Kbox(1:length(RTk_app), iRTk) = RTk_app;
    end

    groups = {'dir1', 'dir2', 'dir3', 'dir4', 'dir5', 'dir6', 'dir7', 'dir8'};

    categories = {'dir1', 'dir2', 'dir3', 'dir4', 'dir5', 'dir6', 'dir7', 'dir8'};
    
    positions_S = (1:length(categories)) - 0.5; 
    positions_K = (1:length(categories)) + 0.5;

    figure
    % boxplot(RT_Sbox, groups, 'Colors', 'b','Positions',positions_S, 'Widths', 0.2);
    boxplot(RT_Sbox, groups, 'Colors', 'b','Widths', 0.2);
    hold on;
    boxplot(RT_Kbox, groups, 'Colors', 'g', 'Positions',positions_K, 'Widths', 0.2);
    xticks(1.25:1:8.25);
    set(gca, 'XTickLabel', categories);
    for ic=1.75:1:num_dir
        xline(ic, '--k', 'LineWidth', 0.5);
    end
    % Creazione di oggetti fittizi per la legenda
    h1 = plot(nan, nan, 'b'); 
    h2 = plot(nan, nan, 'g');
    legend([h1 h2], {'Monkey - S', 'Monkey - K'}, 'Location', 'northwest');
    % set(legend, 'BoxFace', struct('ColorType','truecoloralpha', 'ColorData', uint8([255; 255; 255; 255])));
    % legend('boxoff');
    % set(gca, 'XTickLabelRotation', 45);
    cat_plot = cellfun(@(x, y) [x, '\newline', y], categories, infoNaN_plot, 'UniformOutput', false);
    set(gca, 'XTickLabel', cat_plot);
    ...
    RT_boxplot_path = strcat(dir_path,'\RT_Boxplot');
    if ~exist(RT_boxplot_path,'dir')
        mkdir(RT_boxplot_path)
    end
    name_fig = fullfile(RT_boxplot_path,strcat(condition_name{cd},'_boxplot.png'));
    saveas(gcf,name_fig,'png');
end
...
for cd=1:num_cond
    ...
    for dir = 1:num_dir
        ...
        plot(MK_data_x, MK_data_y, 'g');
        hold on;
        labeltitle = strcat('Dir ', num2str(dir));
        title(labeltitle)
        label_text = strcat('Dir ', num2str(dir)); 
        text(MS_data_x(end), MS_data_y(end), label_text, ...
            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'right', 'FontSize', 10, 'Color', 'b');
...
for cd=1:num_cond
    ...
    for dir = 1:num_dir
        MS_data_x = MS(cd).(strcat('dir', num2str(dir)))(1,:);
        RT_S_data = RT_Smean(cd).(strcat('dir', num2str(dir)))(1,:);
        RT_S_xy = find(time-abs(RT_S_data)>0,1,'First');
        MS_data_y = MS(cd).(strcat('dir', num2str(dir)))(2,:);   

        MK_data_x = MK(cd).(strcat('dir', num2str(dir)))(1,:);
        RT_K_data = RT_Kmean(cd).(strcat('dir', num2str(dir)))(1,:);
