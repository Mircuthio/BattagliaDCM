%% function TEST_BATTAGLIA_TimeVmax_statistic_AllSESSION.m
clear; close all;
par.irng = 10;
rng(par.irng)
...
for cd=1:num_cond+1
    figure;
    for ndir=1:num_dir
        Time_qq = 1000*TimeMax_total(cd).(strcat('dir',num2str(ndir)));
        [h_ks, p_ks] = kstest((Time_qq - mean(Time_qq)) / std(Time_qq));
    ...
    Time_qq_path = strcat(dir_path,'Time_Statistic',filesep,'QQ');

    par.savePlotEpsPdfMat.dir_png = strcat(Time_qq_path,'',filesep,'PNGs',filesep,'');
    par.savePlotEpsPdfMat.dir_pdf = strcat(Time_qq_path,'',filesep,'PDFs',filesep,'');
    par.savePlotEpsPdfMat.dir_mat = strcat(Time_qq_path,'',filesep,'MATfiles',filesep,'');

    name_fig = strcat('QQ','cond',num2str(cd));
    par.savePlotEpsPdfMat.file_name = name_fig;
    savePlotEpsPdfMat(gcf,par.savePlotEpsPdfMat)

    save(fullfile(Time_qq_path,sprintf('KS_irng%d',par.irng)), 'KS');
    close all
end
%% TEST di Friedman sui 4 gruppi
FriedTest = struct();
for ndir=1:num_dir
    Time_dirFried = {TimeMax_total.(strcat('dir',num2str(ndir)))};
...
Time_Fried_path = strcat(dir_path,'Time_Statistic',filesep,'4Group');

par.savePlotEpsPdfMat.dir_png = strcat(Time_Fried_path,'',filesep,'PNGs',filesep,'');
par.savePlotEpsPdfMat.dir_pdf = strcat(Time_Fried_path,'',filesep,'PDFs',filesep,'');
par.savePlotEpsPdfMat.dir_mat = strcat(Time_Fried_path,'',filesep,'MATfiles',filesep,'');

name_fig = strcat('Fried_4Group_','dir',num2str(ndir));
par.savePlotEpsPdfMat.file_name = name_fig;
savePlotEpsPdfMat(gcf,par.savePlotEpsPdfMat)

save(fullfile(Time_Fried_path,sprintf('FriedTest_irng%d',par.irng)), 'FriedTest');
%% TEST di Mann-Whitney & Wilcoxon
alpha = 0.05;
p_value = struct();
for ndir=1:num_dir
    Time_dir = {TimeMax_total.(strcat('dir',num2str(ndir)))};
...
for ndir=1:num_dir
    ...
    title(strcat('Time of Maximum Velocity significance \newline',...
        '                               dir',num2str(ndir)));
    set(gca, 'FontSize', 9);
end
...
Time_2Group_path = strcat(dir_path,'Time_Statistic',filesep,'2Group');

par.savePlotEpsPdfMat.dir_png = strcat(Time_2Group_path,'',filesep,'PNGs',filesep,'');
par.savePlotEpsPdfMat.dir_pdf = strcat(Time_2Group_path,'',filesep,'PDFs',filesep,'');
par.savePlotEpsPdfMat.dir_mat = strcat(Time_2Group_path,'',filesep,'MATfiles',filesep,'');
...
save(fullfile(Time_2Group_path,sprintf('WillMann_irng%d',par.irng)), 'p_value');
close all;
...
for ndir=1:num_dir
    ...
    yticklabels(Groups_nameY)
    title(strcat('                              All Sessions',...
        '\newline Time of Maximum Velocity significance',' dir',num2str(ndir)));
    set(gca, 'FontSize', 14);
    set(gcf, 'WindowState', 'maximized');
