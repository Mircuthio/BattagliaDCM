%% function TEST_BATTAGLIA_RT_Statistics_AllSESSION_Server
clear; close all;
par.irng = 10;
rng(par.irng)
...
% Leggo tutti i file nella directory
...
for idsess = 1:length(session_name)
    ...
    for cd=1:num_cond
        for ndir=1:num_dir
            ...
            RT_Smean(cd).(strcat('dir',num2str(ndir))) = mean(RT_Sdir,'omitnan');
            RT_Kmean(cd).(strcat('dir',num2str(ndir))) = mean(RT_Kdir,'omitnan');
            RT_Sstd(cd).(strcat('dir',num2str(ndir))) = std(RT_Sdir,'omitnan');
            RT_Kstd(cd).(strcat('dir',num2str(ndir))) = std(RT_Kdir,'omitnan');
        end
    end
    ...
    RT_session(idsess).cond = RT_statistic;
...
for cd=1:num_cond+1
    figure;
    for ndir=1:num_dir
        RT_qq = 1000*RT_total(cd).(strcat('dir',num2str(ndir)));
        [h_ks, p_ks] = kstest((RT_qq - mean(RT_qq)) / std(RT_qq));
    ...
    RT_qq_path = strcat(dir_path,'RT_Statistic',filesep,'QQ');

    par.savePlotEpsPdfMat.dir_png = strcat(RT_qq_path,'',filesep,'PNGs',filesep,'');
    par.savePlotEpsPdfMat.dir_pdf = strcat(RT_qq_path,'',filesep,'PDFs',filesep,'');
    par.savePlotEpsPdfMat.dir_mat = strcat(RT_qq_path,'',filesep,'MATfiles',filesep,'');

    name_fig = strcat('QQ','cond',num2str(cd));
    par.savePlotEpsPdfMat.file_name = name_fig;
    savePlotEpsPdfMat(gcf,par.savePlotEpsPdfMat)

    save(fullfile(RT_qq_path,sprintf('KS_irng%d',par.irng)), 'KS');
    close all
end
%% TEST di Friedman sui 4 gruppi
FriedTest = struct();
for ndir=1:num_dir
    RT_dirFried = {RT_total.(strcat('dir',num2str(ndir)))};
...
figure
if FriedTest.p <= 0.0001
    h = heatmap(FriedGroup2,FriedGroup1,[FriedTest.p], 'CellLabelFormat', '%.3e', 'ColorLimits', [0 1]);
else
...
set(gcf, 'WindowState', 'maximized');
RT_Fried_path = strcat(dir_path,'',filesep,'RT_Statistic',filesep,'4Group');

par.savePlotEpsPdfMat.dir_png = strcat(RT_Fried_path,'',filesep,'PNGs',filesep,'');
par.savePlotEpsPdfMat.dir_pdf = strcat(RT_Fried_path,'',filesep,'PDFs',filesep,'');
par.savePlotEpsPdfMat.dir_mat = strcat(RT_Fried_path,'',filesep,'MATfiles',filesep,'');

name_fig = strcat('Fried_4Group_','dir',num2str(ndir));
par.savePlotEpsPdfMat.file_name = name_fig;
savePlotEpsPdfMat(gcf,par.savePlotEpsPdfMat)

save(fullfile(RT_Fried_path,sprintf('FriedTest_irng%d',par.irng)), 'FriedTest');
%% TEST di Mann-Whitney & Wilcoxon
alpha = 0.05;
p_value = struct();
for ndir=1:num_dir
    RT_dir = {RT_total.(strcat('dir',num2str(ndir)))};
            ...
            if ~isnan(curr_pvalue(i, j))
                ...
                elseif curr_pvalue(i, j) <= 0.5
                    if curr_pvalue(i, j) <= 0.0001
                        text(j, i, num2str(curr_pvalue(i, j),'%.3e'), 'HorizontalAlignment', 'center', ...
...
RT_2Group_path = strcat(dir_path,'',filesep,'RT_Statistic',filesep,'2Group');

par.savePlotEpsPdfMat.dir_png = strcat(RT_2Group_path,'',filesep,'PNGs',filesep,'');
par.savePlotEpsPdfMat.dir_pdf = strcat(RT_2Group_path,'',filesep,'PDFs',filesep,'');
par.savePlotEpsPdfMat.dir_mat = strcat(RT_2Group_path,'',filesep,'MATfiles',filesep,'');
...
save(fullfile(RT_2Group_path,sprintf('WillMann_irng%d',par.irng)), 'p_value');
close all;
