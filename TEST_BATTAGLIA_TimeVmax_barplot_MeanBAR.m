%% function TEST_BATTAGLIA_TimeVmax_barplot_MeanBAR.m
clear; close all;
par.irng = 10;
rng(par.irng)
...
for cd=1:num_cond
    for ndir = 1:num_dir
        TimeSG_S(cd).(strcat('dir',num2str(ndir))) = mean(TS_smoothSG_Max(cd).(strcat('dir',num2str(ndir))));
        TimeSG_K(cd).(strcat('dir',num2str(ndir))) = mean(TK_smoothSG_Max(cd).(strcat('dir',num2str(ndir))));
        TimeSG_Sstd(cd).(strcat('dir',num2str(ndir))) = std(TS_smoothSG_Max(cd).(strcat('dir',num2str(ndir))));
        TimeSG_Kstd(cd).(strcat('dir',num2str(ndir))) = std(TK_smoothSG_Max(cd).(strcat('dir',num2str(ndir))));
...
timeS_SGplotMaxstd = (cell2mat(struct2cell(TimeSG_Sstd')))';
timeK_SGplotMaxstd = (cell2mat(struct2cell(TimeSG_Kstd')))';
...
Tmax_plotSGstd = [timeS_SGplotMaxstd(1,:);timeK_SGplotMaxstd(2,:);timeS_SGplotMaxstd(3,:);timeK_SGplotMaxstd(3,:)];

max_y = max(max(Tmax_plotSG))+max(max(Tmax_plotSGstd));
...
Tmax_barplot_path = strcat(dir_path,'\TimeMax\MeanDir\',num2str(windowSize),'wind\');
